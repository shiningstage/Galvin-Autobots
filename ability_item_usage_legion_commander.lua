...
function AbilityUsageThink()
	...
	castOODesire = ConsiderOverwhelmingOdds();
	...
	if ( castOODesire > 0 ) 
	then
		npcBot:Action_UseAbility( abilityOO );
		return;
	end
...
function ConsiderOverwhelmingOdds()
	...
	if ( not abilityOO:IsFullyCastable() ) then 
	...
	if mutil.IsValidTarget(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) 
		and mutil.IsInRange(npcTarget, npcBot, nRadius) and mutil.CanKillTarget(npcTarget, nDamage, DAMAGE_TYPE_MAGICAL)
	then
		return BOT_ACTION_DESIRE_VERYHIGH;
	end
	
	-- If we're pushing or defending a lane and can hit 4+ creeps, go for it (edited to 3 creeps)
	if  ( mutil.IsDefending(npcBot) or mutil.IsPushing(npcBot) ) and npcBot:GetMana() / npcBot:GetMaxMana() > 0.5
	then
