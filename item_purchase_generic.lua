...
local function GeneralPurchase()
	...
	--buy the item if we have the gold  (bot get gold >= itemcost)
...
function ItemPurchaseThink()  
	...
	if  ( GetGameMode() ~= 23 and DotaTime() > 20*60 and DotaTime() > fullInvCheck + 2.0 
		...
		or ( GetGameMode() == 23 and DotaTime() > 1*60 and DotaTime() > fullInvCheck + 2.0  )
	then
				...
				if itemSlot >= 0 and itemSlot <= 8 then
					...
					elseif item == "item_magic_wand" then
						if bot.buildHoly == false  then
	     				...
	     				elseif item == 											"item_recipe_magic_wand" then
						if bot.buildHoly == false  then
							slotToSell = itemSlot;
							break;
						end	
					elseif item == "item_branches" then
						if bot.buildHoly == false  then
							slotToSell = itemSlot;
							break;
						end	
	...
	--Sell non BoT boots when have BoT, edited removed dotatime() > 30 *60, check if turbo or not
	if (GetGameMode() ~= 23 and ( bot:DistanceFromFountain() == 0 or bot:DistanceFromSecretShop() == 0 ) ) 
		or GetGameMode() == 23 then
			if items.HasItem( bot, "item_travel_boots") or items.HasItem( bot, "item_travel_boots_2") and
	...
	--------------------------------
	-- add aghanims_shard to list of buy items
	
	if addAghanimsShard == false and GetItemStockCount( "item_aghanims_shard" ) > 0 then
		bot.currentComponentToBuy = nil;	
		bot.currListItemToBuy[#bot.currListItemToBuy+1] = 'item_aghanims_shard';
		addAghanimsShard = true;
		--bot.itemToBuy[#bot.itemToBuy+1] = 'item_aghanims_shard';
	end


	--Fill purchase table with aghanims_shard (working but green'd out for now)
	-- if DotaTime() > (0*60 + 30) and addAghanimsShard == false then
		-- bot.itemToBuy = {
	...
	-- edited to make sure to buy moon shard only if not yet consumed and travel boots 2 only if no travel boots 2
	--Fill purchase table with super late game item
	if #bot.itemToBuy == 0 and addVeryLateGameItem == false then
	
		bot.itemToBuy = {}
		
		if bot:HasModifier("modifier_item_moon_shard_consumed") == false and items.HasItem(bot, 'item_travel_boots_2') == false then
			bot.itemToBuy = {
			'item_travel_boots_2',
			'item_moon_shard',
			}
		...
		if bot:HasModifier("modifier_item_moon_shard_consumed") == false then
			bot.itemToBuy = {
			'item_moon_shard',
			}
		end
		...
		if items.HasItem(bot, 'item_travel_boots_2') == false then
			bot.itemToBuy = {
			'item_travel_boots_2',
			}
		end	
