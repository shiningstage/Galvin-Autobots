...
ItemUsageModule.Use['item_tango'] = function(item, bot, mode, extra_range)
	--- made a part of code dead by changing action desire absolute to none (for both parts below)
	...
	if DotaTime() > -80 and DotaTime() < 0 and bot:DistanceFromFountain() == 0 and role.CanBeSupport(bot:GetUnitName())
	   and bot:GetAssignedLane() ~= LANE_MID and tCharge > 2 and DotaTime() > giveTime + 2.0 then
		...
		if target ~= nil then
			giveTime = DotaTime();
			return BOT_ACTION_DESIRE_NONE, target, 'unit';
		end
	elseif bot:GetActiveMode() == BOT_MODE_LANING and role.CanBeSupport(bot:GetUnitName()) and tCharge > 1 and DotaTime() > giveTime + 2.0 then
		...
		do
			...
			then
				giveTime = DotaTime();
				return BOT_ACTION_DESIRE_NONE, ally, 'unit';
			end
		end
	end
...
ItemUsageModule.Use['item_mekansm'] = function(item, bot, mode, extra_range)
	...
	local nRadius = 1200;
	local nHeal = 275;
--- increased coefficient to 0.5 -> 0.7 * bot:getmaxhealth so that the heal will be earlier
	if mutil.IsRetreating(bot) then
		local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE);
		if #enemies > 0 and bot:GetHealth() + nHeal <= 0.7*bot:GetMaxHealth() then
			return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
		end
	end
	...
	if mutil.IsInTeamFight(bot, 1300) then
		...
		local allies = bot:GetNearbyHeroes(nRadius, false, BOT_MODE_NONE);
		for i=1, #allies do
			if allies[i]:GetHealth() + nHeal <= 0.7*allies[i]:GetMaxHealth() then
				n_low_hp = n_low_hp + 1;
			end
		end
		if n_low_hp > 0 then
...
ItemUsageModule.Use['item_holy_locket'] = function(item, bot, mode, extra_range)
	...
	if charges > 5 
		and mutil.IsGoingOnSomeone(bot)
--raised hp ratio to 0.55->0.75 and mp ratio 0.25->0.5 for this to use
		and hp_ratio < 0.75 and mp_ratio < 0.5
	then
		local enemies = bot:GetNearbyHeroes(800, true, BOT_MODE_NONE);
		if #enemies > 0 
		then
	...
	---increased ratio before item is used
	if charges >= 10 then
		local allies=bot:GetNearbyHeroes(nCastRange,false,BOT_MODE_NONE);
		for i=1, #allies do
			if allies[i]:GetUnitName() ~= bot:GetUnitName() 
			...
			then
				...
				if  ( ( mode2 == BOT_MODE_RETREAT and allies[i]:GetHealth() < 0.5 * allies[i]:GetMaxHealth() ) 
						or ( allies[i]:GetHealth() < 0.4 * allies[i]:GetMaxHealth() 
...
ItemUsageModule.Use['item_bloodstone'] = function(item, bot, mode, extra_range)
	return ItemUsageModule.Use['item_satanic'](item, bot, mode, extra_range);
end
...
ItemUsageModule.Use['item_disperser'] = function(item, bot, mode, extra_range)
	return ItemUsageModule.Use['item_diffusal_blade'](item, bot, mode, extra_range);
end
...
--item_pavise
ItemUsageModule.Use['item_pavise'] = function(item, bot, mode, extra_range)
...
ItemUsageModule.Use['item_solar_crest'] = function(item, bot, mode, extra_range)
	return ItemUsageModule.Use['item_pavise'](item, bot, mode, extra_range);
end
