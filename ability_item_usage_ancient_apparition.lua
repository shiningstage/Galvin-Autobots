...
function ConsiderChillingTouch()
	...
	-- If we're going after someone, edited to ncastrange+240 instead of 200
	if mutil.IsGoingOnSomeone(npcBot)
	then
		local npcTarget = npcBot:GetTarget();
		if mutil.IsValidTarget(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nCastRange+240)
		then
