...
local itemScepter = nil;
local allieslist = GetTeamPlayers(GetTeam());
local aghself = 0;
...
function AbilityUsageThink()
	...
	aghsavail = IsItemAvailable("item_ultimate_scepter")
	if aghsavail ~= nil then
	castASDesire, itemScepter, castASTarget = considerAghanimsScepter();
	end
	...
	if ( castASDesire > 0 ) 
	then
		npcBot:Action_UseAbilityOnEntity( itemScepter, castASTarget );
		return;
	end
...
function ConsiderFleshGolem()
	...
	then
		...
		for _,npcEnemy in pairs( tableNearbyEnemyHeroes )
		do
			if ( npcBot:WasRecentlyDamagedByHero( npcEnemy, 1.0 ) and npcBot:GetHealth() / npcBot:GetMaxHealth() < 0.6 ) 
			then
				return BOT_ACTION_DESIRE_HIGH;
			end
		end
	...
	then
		local npcTarget = npcBot:GetTarget();
		if ( mutil.IsValidTarget(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nRadius-400) and npcBot:GetHealth() / npcBot:GetMaxHealth() < 0.65 ) 
		then
			return BOT_ACTION_DESIRE_HIGH;
		end
	end
...
function considerAghanimsScepter()
...
local allies= npcBot:GetNearbyHeroes(900,false,BOT_MODE_NONE);
	...
	if aghself == 0 then
		aghself = 1
		return BOT_ACTION_DESIRE_ABSOLUTE, aghscepter, GetBot();
	end
	...
	for i=1, #allies do
		...
		if mutil.IsValidTarget(allies[i]) 
			and	not (allies[i]:HasModifier('modifier_item_ultimate_scepter_consumed_alchemist'))
			and mutil.CanCastOnMagicImmune(allies[i])
			and allies[i] ~= GetBot()
		then
			return BOT_ACTION_DESIRE_ABSOLUTE, aghscepter, allies[i];
		end
	end
