...
function ConsiderChrono()
	...
	-- If we're pushing or defending a lane and can hit 4+ creeps, go for it (i changed this to 3)
...
function ConsiderRadiantBind()
	...
	-- Make sure it's castable (copy pasted from other part)
	if ( 
		-- not npcBot:HasModifier("modifier_keeper_of_the_light_spirit_form") 
		abilityRB:IsHidden() or 
		not abilityRB:IsFullyCastable() 
	) 
	then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
